...
class VisualFeatureExtractor(nn.Module):
    ...
    def forward(self, images):
        ...
        if self.model_name == 'clip':
            #初步读取一张图片
            # visual_features = self.model.encode_image(images).float().squeeze(0)
            visual_features = self.model.encode_image(images).float()
            avg_features = visual_features
            try:
                avg_features = avg_features.squeeze(2)
                avg_features = avg_features.squeeze(2)
            except:
                pass
        else:
            ...
            avg_features = self.avg_func(visual_features)
            try:
                avg_features = avg_features.squeeze(2)
                avg_features = avg_features.squeeze(2)
            except:
